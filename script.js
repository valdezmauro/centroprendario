...
async function boot(){
  const data = awat fetch('data.json').then(r=>r.json());
...
// Parsear string con puntos (.) de miles y coma (,) decimal a número
...
// Adjunta formateo con miles (es-AR) a un <input type="text">
function attachThousandsFormatter(input) {
  const format = () => {
    let v = input.value ?? "";
    // permitimos dígitos y UNA coma
    v = v.replace(/[^\d,]/g, "");
    const parts = v.split(",");
    let intPart = parts[0] || "";
    const decPart = parts.length > 1 ? parts[1] : undefined;

    intPart = intPart.replace(/^0+(?=\d)/, ""); // evita 0001 → 1
  ...
  // Asegurar formateo final al salir
...
function makeCard(b) {
  ...
  const render = () => {
    ...
    // Si el bloque NO es "2025 / 2015", ocultamos plazos > 36 (opcional)
    const rows =
      b.titulo.trim() === "2025 / 2015"
        ? b.coeficientes
        : b.coeficientes.filter((row) => Number(row.plazo) <= 36);
